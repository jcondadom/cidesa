...
class radarventasActions extends autoradarventasActions
{
  ...
  public function executeAjax()
  {
	...
	$c->add(CuentaxcobrarClientesPeer::ID,$clasemodelo->getFaclienteId());
  ...
  {
	if ($clasemodelo->getFaclienteId()!="")
	{
		$c = new Criteria();
		$c->add(CuentaxcobrarClientesPeer::ID,$clasemodelo->getFaclienteId());
		$resultado = CuentaxcobrarClientesPeer::doSelect($c);
		//echo $resultado['0']->getCuentaxcobrar();
		if ($resultado['0']->getCuentaxcobrar()>0)
		{
             	        $this->setFlash('error', 'El cliente tiene cuentas por pagar');                    
		...
		else
		{
			if ($clasemodelo->getId()=="")
			{
				...
				$radarventas->setReffac($clasemodelo->getReffac());
				$radarventas->save();
