...
class Ciudadanos {
  ...
  /*    foreach($griditems[1] as $item) {
   ...
   public static function getFacliente($ruta='')
    {
      ...
      if($ruta!='')
      {
          $c->add(RutaclientePeer::RUTA_ID,$ruta);
          $c->addJoin(FaclientePeer::ID,RutaclientePeer::FACLIENTE_ID);
          $faclientes = FaclientePeer::doSelect($c);
	  //$facliente[0]='';
	  if($faclientes)
          {
		foreach($faclientes as $f)
                {
		  $facliente[$f->getId()] = $f->getNompro();
		}
	  }
	   //print_r($facliente);exit();
      } 
      return $facliente;
    }
}
