...
abstract class BaseCatproductos extends BaseObject  implements Persistent {
  ...
  public function getId()
  {
    ...
    if ($this->codart !== $v) {
        $this->codart = $v;
        $this->modifiedColumns[] = CatproductosPeer::CODART;
      }
    ...
    if ($this->desart !== $v) {
        $this->desart = $v;
        $this->modifiedColumns[] = CatproductosPeer::DESART;
      }
    ...
    if ($this->id !== $v) {
        $this->id = $v;
        $this->modifiedColumns[] = CatproductosPeer::ID;
      }
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      throw new PropelException("Error populating Catproductos object", $e);
    }
  }
  ...
  public function get($m, $a)
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			CatproductosPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CatproductosPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += CatproductosPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CatproductosPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CatproductosPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CatproductosPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CatproductosPeer::CODART)) $criteria->add(CatproductosPeer::CODART, $this->codart);
		if ($this->isColumnModified(CatproductosPeer::DESART)) $criteria->add(CatproductosPeer::DESART, $this->desart);
		if ($this->isColumnModified(CatproductosPeer::ID)) $criteria->add(CatproductosPeer::ID, $this->id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CatproductosPeer::ID, $this->id);
	...
	{
		if (self::$peer === null) {
			self::$peer = new CatproductosPeer();
		}
		return self::$peer;
	}
		...
		if ($this->collMetass === null) {
			if ($this->isNew()) {
			   $this->collMetass = array();
				...
				$criteria->add(MetasPeer::CATPRODUCTOS_ID, $this->getId());
	...
	{
		$this->collMetass[] = $l;
		$l->setCatproductos($this);
	}


	
	public function getMetassJoinVendedores($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseMetasPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collMetass === null) {
			if ($this->isNew()) {
				...
				$this->collMetass = MetasPeer::doSelectJoinVendedores($criteria, $con);
			}
		} else {
