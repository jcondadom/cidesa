...
abstract class BaseViaregente extends BaseObject  implements Persistent {
  ...
  public function hydrate(ResultSet $rs, $startcol = 1)
  {
    try {
      ...
      $this->dirente = $rs->getString($startcol + 4);

      $this->telente = $rs->getString($startcol + 5);

      $this->corente = $rs->getString($startcol + 6);

      $this->actecoente = $rs->getString($startcol + 7);
  ...
  public function get($m, $a)
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getDirente(),
			$keys[5] => $this->getTelente(),
			$keys[6] => $this->getCorente(),
			$keys[7] => $this->getActecoente(),
			$keys[8] => $this->getId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setDirente($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setTelente($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setCorente($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setActecoente($arr[$keys[7]]);
